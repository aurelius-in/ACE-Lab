  ...
  const [tip, setTip] = useState<string | null>(null);
	...
	useEffect(() => {
		...
		const compile = (g: WebGL2RenderingContext, type: number, src: string) => {
			...
			if (!g.getShaderParameter(sh, g.COMPILE_STATUS)) { console.error(g.getShaderInfoLog(sh)); }
			return sh;
		};
		const vs = compile(gl, gl.VERTEX_SHADER, VERT_SRC as string);
		...
		const makeTex = (g: WebGL2RenderingContext) => {
		...
		upload(gl, media.primary?.src || '/ace-lab.webp', tex0);
		if (media.secondary) upload(gl, media.secondary.src, tex1);

		// offscreen framebuffer for post
		const rtTex = makeTex(gl);
		const fbo = gl.createFramebuffer()!;
		const resizeRT = () => { gl.bindTexture(gl.TEXTURE_2D, rtTex); gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA, c.width, c.height,0,gl.RGBA,gl.UNSIGNED_BYTE,null); gl.bindFramebuffer(gl.FRAMEBUFFER, fbo); gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, rtTex, 0); gl.bindFramebuffer(gl.FRAMEBUFFER, null); };
		...
		const loop = () => {
			...
			const wantCrossNow = effect.id === 'crosszoom' && !!media.secondary;
			const mix = wantCrossNow ? mixAt(play.t) : 1.0;
